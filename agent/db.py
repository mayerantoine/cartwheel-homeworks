...
def list_orders_for_user(
    conn: sqlite3.Connection, user_id: int, limit: int | None = 20
) -> list[Order]:
    if limit is None:
        rows = conn.execute(
...
def list_all_orders(conn: sqlite3.Connection) -> list[Order]:
    """Return every order, newest first, for support-scoped searches."""
    rows = conn.execute(
